...
import { decisions, getActiveDecisions, getCompletedDecisions, users, currentUser } from '../../data/mockData';
...
export const DecisionsList = () => {
    ...
    const filteredAndSortedDecisions = useMemo(() => {
        ...
        if (creatorFilter !== 'all') {
            result = result.filter(d => d.creatorId === creatorFilter);
        }

        // Vote filter
        if (voteFilter === 'voted') {
            result = result.filter(d => d.options.some(opt => opt.voters.includes(currentUser.id)));
        } else if (voteFilter === 'not_voted') {
            result = result.filter(d => 
                d.status === 'active' && 
                !d.options.some(opt => opt.voters.includes(currentUser.id))
            );
        }
        ...
        switch (sortBy) {
            ...
            case 'participation':
                result.sort((a, b) => {
                    const aRate = a.options.reduce((sum, opt) => sum + opt.votes, 0) / a.participants.length;
                    const bRate = b.options.reduce((sum, opt) => sum + opt.votes, 0) / b.participants.length;
                    return bRate - aRate;
                });
                break;
        }
        ...
        return result;
    }, [filter, sortBy, creatorFilter, voteFilter, urgentOnly]);

    const activeCount = getActiveDecisions().length;
    const completedCount = getCompletedDecisions().length;
